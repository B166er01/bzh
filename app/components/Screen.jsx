"use client";
import React from "react";
import BreizhThe from "./BreizhThe";
import MagneticButtons from "./MagneticButtons";

const Screen = () => {
  return (
    <div id="blue" className="relative w-full h-screen bg-myWhite">
      <BreizhThe />
      {/* <MagneticButtons>
        <button className="relative  border-myRed h-[150px] text-3xl uppercase rounded-[50%] cursor-pointer w-[300px] bg-myWhite border-4 transition-colors font-bold text-myRed hover:text-myWhite hover:bg-myRed top-1/2 left-1/2 ">
          découvrez le
        </button>
      </MagneticButtons>
      <button className="relative">gy y uygyyu uy </button> */}

      <MagneticButtons>
        <button className="relative -rotate-6 border-myRed h-[150px] text-3xl uppercase rounded-[50%] cursor-pointer w-[300px] bg-myWhite border-4 transition-colors font-bold text-myRed hover:text-myWhite hover:bg-myRed top-[50%] left-1/2 ">
          découvrez le
        </button>
      </MagneticButtons>
    </div>
  );
};

export default Screen;
